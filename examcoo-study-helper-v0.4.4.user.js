...
// @version      0.4.4
// @description  给老婆用的练习小助手：自动填写做过的题，保存结果页正确答案，还能用 AI 补充未作答题；最后由你亲自检查和提交。
// @match        *://examcoo.com/*
// @match        *://*.examcoo.com/*
// @match        *://examcoo.cn/*
// @match        *://*.examcoo.cn/*
...
(function () {
  ...
  const SETTINGS_KEY = 'examcoo_local_answer_settings_v1';
  const AI_KEY = 'examcoo_deepseek_config_v1';
  const settings = Object.assign({ autoRemember: true }, GM_getValue(SETTINGS_KEY, {}));
  const aiConfig = Object.assign({ apiKey: '', model: 'deepseek-v4-flash' }, GM_getValue(AI_KEY, {}));
  let bank = GM_getValue(BANK_KEY, {});
  let questions = [];
  let mapped = [];
  let statusNode = null;
  let panelRoot = null;
  ...
  function getPageMode() {
    ...
    if (/\/editor\/do\/recur(?:\/|$)/.test(path)) return 'result';
    if (/\/editor\/do\/exam(?:\/|$)/.test(path)) return 'exam';
    return 'unknown';
  }
  ...
  function mapQuestions() {
    ...
    mapped = questions.map((question, qIndex) => {
      ...
      const exactInputs = exactContainer
        ? [...exactContainer.querySelectorAll('input.dijitCheckBoxInput[name$="_option"]')]
        : [];
  ...
  function applySavedAnswers(entries) {
    ...
    try {
      entries.forEach(({ question, inputs }) => {
        ...
        const matchedInputs = inputs.filter((input) => record.answers.some((answer) => matchInput(input, answer)));
        if (matchedInputs.length !== record.answers.length) {
          unmapped += 1;
          details.push({ question: question.text, reason: '答案文字未匹配到当前选项', savedAnswers: record.answers, pageOptions: inputs.map(optionText) });
          return;
        }
  ...
  function saveSelections(silent = false) {
    if (!mapped.length) mapQuestions();
    let saved = 0;
    mapped.forEach(({ question, inputs }) => {
      const selected = inputs.filter((input) => input.checked);
      if (!selected.length) return;
      const answers = selected.map(optionText).map((text) => {
        const normalized = normalize(text);
        const source = question.options.find((option) => normalize(option) === normalized);
    ...
    updateStatus(silent ? undefined : `已将 ${saved} 道手动选择保存到本地；这些不是网站官方参考答案`);
  }

  let saveTimer = null;
  function bindRememberHandlers() {
    mapped.forEach(({ inputs }) => inputs.forEach((input) => {
      if (input.dataset.examcooHelperBound) return;
      input.dataset.examcooHelperBound = '1';
      input.addEventListener('change', () => {
        if (!settings.autoRemember || isFilling) return;
        clearTimeout(saveTimer);
        saveTimer = setTimeout(() => saveSelections(true), 350);
      });
    }));
  }
  ...
  function importBank() {
    ...
    input.addEventListener('change', async () => {
      try {
        ...
        bank = Object.assign({}, bank, incoming);
        GM_setValue(BANK_KEY, bank);
        updateStatus(`导入成功，本地题库共 ${Object.keys(bank).length} 题`);
      } catch (error) {
  ...
  async function answerWithAi() {
    ...
    try {
      ...
      const applied = applySavedAnswers(pending);
      updateStatus(`AI 已分析 ${accepted} 题并勾选 ${applied.filled} 题；格式校验未通过 ${rejected} 题。已作答题未被修改，请人工复核。`);
    } catch (error) {
  ...
  function createPanel() {
    ...
    root.innerHTML = `
      ...
      <div class="panel">
        <div class="title">老婆专用·练习小助手 · <span id="mode"></span></div>
        <div class="row">
          <button id="scan">重新识别</button><button id="fill" data-mode="exam">回填已保存</button>
          <button id="save" data-mode="exam">保存已选答案到本地</button><button id="ai-answer" data-mode="exam">AI分析未作答题</button>
          <button id="harvest" data-mode="result">保存本页正确答案</button>
          <button id="import">导入题库</button><button id="export">导出题库</button>
        </div>
        <label data-mode="exam"><input id="remember" type="checkbox"> 自动记忆手动选择</label>
    ...
    root.getElementById('remember').checked = settings.autoRemember;
    root.getElementById('remember').addEventListener('change', (event) => {
      settings.autoRemember = event.target.checked;
      GM_setValue(SETTINGS_KEY, settings);
      updateStatus(settings.autoRemember ? '已开启自动记忆' : '已关闭自动记忆');
    });
    ...
    root.getElementById('save').addEventListener('click', () => saveSelections(false));
